...
recovery gate in issue #172 passes. `/vsphere-key-providers` manages
...
for imported public leaves. Authenticated status returns only service/store health and nullable per-provider lifecycle
counts; unavailable evidence is never represented as zero. Disabling the service preserves the operational store.
