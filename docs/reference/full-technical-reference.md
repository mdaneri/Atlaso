...
`op run --environment` command to carry the same opaque ID. It also requires a fresh, non-secret per-invocation nonce
generated by the bridge to appear in both the `op.exe` and child command lines; this permits the documented command to
