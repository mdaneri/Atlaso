...
captures valid IPv4 and IPv6 gateways before those management-only fields clear. In the same database transaction it
...
Web Terminal uses that same applied projection for management-plane page, ticket, and WebSocket eligibility. Applied
flagged access physical and VLAN bindings are mandatory terminal listeners even if pending desired rows would no longer
select them. Explicit additional listeners continue to derive from Web Terminal desired selection and remain public.
The Network baseline moves this classification only after the protected handoff application commit, so failure,
rollback, cancellation, and desired-state reversion retain the old terminal listener.
...
certificate rotates. The pre-retirement candidate certificate check excludes old-only live addresses served by those
blocks while retaining configured, acquired dynamic, and unrelated secondary candidate addresses. The final
post-retirement observation checks every still-assigned address against the candidate certificate before commit.
...
port. Dedicated and flagged access physical/VLAN management listeners all retain TCP/22 for ordinary
bootstrap-administrator SSH alongside TCP/80, TCP/443, and that configured public port. The flagged-listener rule keeps
its rendered Source Group predicate; unflagged access networks receive no management SSH admission, and root SSH stays
under the independent Appliance Settings policy. A filtered-to-filtered handoff adds that port to both the
transitional ruleset and the final candidate ruleset so
...
and rejects a second default of the same family. The UI/API,
