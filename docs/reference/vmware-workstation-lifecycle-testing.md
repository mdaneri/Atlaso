...
or rotate the service-account token in the 1Password portal, then enter it once at the non-echoing prompt:
...
profile, a command argument, or `OP_SERVICE_ACCOUNT_TOKEN` in the caller. Rotate the token by rerunning the helper with
...
The VMware build, normal-test-VM, deployment, and export entry points use this authentication precedence:

1. Explicit `-OnePasswordServiceAccountTokenFile` DPAPI ciphertext.
2. Explicit `-OnePasswordAccount` desktop authorization.
3. The checkout-local DPAPI token file when present.
4. Existing single-account desktop discovery.

The token is decrypted only in a bounded child and supplied to the immediate SDK or `op run` process. The SDK removes
its environment copy before Environment retrieval. A missing, corrupt, wrong-user, malformed, revoked, or unauthorized
...
through `-OnePasswordEnvironmentId`. The checkout-local service-account token is used automatically when present;
pass `-OnePasswordAccount` only to select desktop authorization. The bridge automatically selects standard GIL-enabled
Windows x64 CPython 3.14; use
...
1Password Python SDK integration and retrieves the value
...
Authorization and Environment retrieval each use the deployment timeout, so a revoked token, ignored desktop approval
prompt, or non-responsive SDK exits without beginning the VMware deployment.
...
independently authoritative. With no explicit selectors, the bridge prefers the checkout-local current-user DPAPI
service-account token before single-account desktop discovery and uses standard GIL-enabled Windows x64 CPython 3.14
registered with the Windows launcher, including current Python
...
schema-v3 role-bound provenance that binds the admitted clean commit, deterministic immutable source-snapshot
inventory, and matching builder identity; normal test-VM cloning and OVF export both verify it before accepting the
source. A
...
Durable normal-test-VM recovery repeats that same narrow registration proof after an earlier provider attempt removed
...
both an immutable source snapshot and the matching builder identity. This
...
root. Existing test VMs retain the prior root and leaf certificates they imported at first boot and cannot satisfy the
current checked-in fingerprint or `-TrustRootCa` on a clean Windows host. Redeploy every normal test VM through its
owning wrapper after a root rotation before using it for current validation. Preserve an existing VM until its owner
can safely replace it, and verify the new public fingerprint before trusting it on Windows.
...
blocks destructive rollback. Before a pre-secret rollback launches removal, it reconciles any exact identity-bound
marker whose atomic rename completed before its caller path was exposed, then durably binds the exact stopped VMX,
...
identity. The wrapper prefers the checkout-local current-user DPAPI service-account token and uses standard GIL-enabled
Windows x64 CPython 3.14 registered with the Windows launcher, including bracketed Python Install Manager architecture
selectors. Use `-OnePasswordServiceAccountTokenFile`, `-OnePasswordAccount`, or `-OnePasswordPython` only when an
explicit override is required. Without a token, zero or multiple desktop accounts fail closed; a missing compatible
runtime always fails before VMware mutation.
...
1Password SDK integration. Service-account authentication does not invoke `DesktopAuth`; explicit account selection
preserves the existing desktop flow. Each explicit password parameter remains independently authoritative; there are
no password prompts, caller-environment fallbacks, repository defaults, plaintext token files, or local `.env` inputs.
...
might be returned directly or inside one matching pair of VMware representation quotes; readiness removes only that
outer pair and rejects unbalanced, nested, or embedded quotes as ambiguous. A hostname mismatch, duplicate static
address, or neighbor entry owned by another running VM fails closed with the relevant exact
...
stopped builder. Once process-tree termination and exact-VM inactivity are proven, stale-only evidence permits
release of the completed builder's reservation. It does **not** make the address eligible for another build:
automatic allocation still skips it, and an explicit request for it fails while the cache entry remains. Release
does not flush the neighbor cache or claim that its MAC belongs to the builder. Current host-interface assignments,
running-VM address use, non-stale neighbors, and unavailable provider evidence still block release.

For an already retained handoff, let the original controller exit before retrying the read-only command. When the
build was launched from an interactive PowerShell session, that session is the controller: after the build returns,
close that session and run recovery from a new session. A valid receipt then permits same-boot recovery without
waiting for stale-only cache entries to disappear. Verify `releasable` before requesting `-Cleanup`; do not edit
reservation JSON. The recovery command retains its active-controller and fresh provider checks.
