...
exact-VMX startup heartbeats and applies a 15-minute default timeout. Use `-PackerStartupTimeoutSeconds` to select a
different bounded start-to-provisioning interval and `-PackerHeartbeatSeconds` to adjust diagnostic frequency. A
