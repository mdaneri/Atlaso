...
The bounded child also authenticates that `op run --environment` carries the same opaque ID as the concealed child-only
handoff and matches a fresh, non-secret per-invocation nonce generated by the bridge in both process command lines.
This supports starting the documented command from an existing PowerShell prompt while rejecting an interactive
`op run` shell that did not launch the bridge.
