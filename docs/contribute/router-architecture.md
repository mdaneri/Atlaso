...
`services/task_log_history.py` owns immutable task chunks and a compact sanitized producer checkpoint. ORM flushes
capture Job and job AuditEvent changes after generated IDs exist; conditional bulk result updates explicitly call the
same capture service inside their successful transaction. The task row serializes producers before checkpoint changes.
Startup initializes retained tasks after schema reconciliation, and the log route reads bounded character windows
without writing snapshots. Job deletion removes the owned chunks and checkpoint.
