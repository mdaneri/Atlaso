...
- Virtualization publication is separate. A clean Windows workstation consumes the exact signed `vX.Y.Z` bundle,
  ...
  assets are byte-idempotent resume points only; any identity or byte collision fails closed, and tooling never invents
  a replacement `rc.N`.
...
- Real Appliance Settings apply stages JSON under `/var/lib/atlaso/apply/appliance-settings/atlaso-settings.json` as the
  ...
  pre-start gate so VMware signer scrub can run concurrently without admitting appliance readiness early. Host timeout
  diagnostics may consume only bounded fixed non-secret first-boot stage identifiers. Commit a durable
