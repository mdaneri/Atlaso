...
- Release publication recovery must use the protected **Publish appliance release** manual dispatch with the exact
  ...
  append one complete OVF asset set to that exact stable release or to an existing prerelease without overwriting;
  recovery must validate its manifest, two payload VMDKs, four-disk descriptor topology, optional byte-equivalent OVA,
  and per-asset size boundary. Stable publication requires the synchronized `vX.Y.Z` tag. Prerelease publication
  requires exactly one annotated SemVer prerelease tag with the same `X.Y.Z` core at the exact clean source commit, and
  the destination GitHub Release must already be published, non-draft, and classified for the selected mode. Require a
  byte-bound build provenance record and resolve the destination repository's tag to that commit before upload.
  Annotated tag creation must
