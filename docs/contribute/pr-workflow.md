...
## Maintainer-selected automatic branch updates

GitHub auto-merge remains a separate explicit maintainer choice per pull request. When it is enabled and `main`
advances, `update-auto-merge-prs.yml` updates only open, non-draft, conflict-free, same-repository pull requests with
auto-merge enabled and a `BEHIND` state. Its update-branch request includes the observed head SHA as
`expected_head_sha`, so a concurrent contributor push rejects the stale update instead of overwriting it. Forks,
drafts, conflicted branches, and pull requests without auto-merge are never updated by this workflow.
After observing the updated head, the workflow requests protected version/CI refresh through the typed repository
dispatch described in [release policy](release-policy.md). It does not grant agents permission to enable auto-merge
or bypass the guarded delivery requirements above.
