...
Atlaso Routing/WAN v1 is intentionally appliance-owned and conservative. Desired state is edited on `/routes-wan`; host mutation happens only through the global `/appliance-apply` `wan` unit.

Atlaso has no `wan` interface role. The `wan` apply-unit name and WAN Simulation UI describe explicit routing, NAT, and impairment behavior only; they must not be used as an interface classification.
