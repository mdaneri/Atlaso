...
| `/routes-wan` — routes, permissions, NAT, policies | Direct-edit Tabulator; Physical Interfaces reference | Four paired fallback tables and rendered validation/config preview | Role-gated desired-state edits and confirmations; enforcement only through global `wan` apply | Pass D/N/K/F/Z/V | E1–E8 |
...
| `/physical-interfaces` — observed and desired interfaces | Direct-edit Tabulator; Physical Interfaces reference | Paired interface fallback with observed and desired values | Role-gated desired-state edits; confirmation for guarded state changes; global `network` apply only | Pass D/N/K/F/Z/V | E1–E8 |
| `/vlan-interfaces` — VLAN desired state | Direct-edit Tabulator; Physical Interfaces reference | Paired VLAN fallback and validation rail | Role-gated desired-state edits; dependent-state validation; global `network` apply only | Pass D/N/K/F/Z/V | E1–E8 |
