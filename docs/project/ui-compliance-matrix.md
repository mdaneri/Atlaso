...
| `/ui/management/ntp` — settings, upstreams, source health | Wizard-backed Tabulator and non-grid settings; ESX Storage/DNS references | Upstream fallback, settings, validation, and config preview | Role-gated desired-state writes; capability-gated NTS client/server controls; unsupported disables controls, while unknown preserves desired state and blocks unsafe apply; NTS server apply keeps CA-before-NTP ordering and Firewall-owned TCP/4460 access; global `ntpd` apply only | Pass D/N/K/F/Z/V | E1–E8 |
