...
| `/vlan-interfaces` — VLAN desired state | Direct-edit Tabulator; Physical Interfaces reference | Paired VLAN fallback, access management UI exposure, and validation rail | Role-gated desired-state edits; management-path and dependent-state validation; global `network` apply only | Pass D/N/K/F/Z/V | E1–E8 |
