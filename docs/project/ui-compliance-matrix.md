...
| `/ui/management/ntp` — settings, upstreams, source health | Wizard-backed Tabulator and non-grid settings; ESX Storage/DNS references | Upstream fallback, settings, validation, and config preview | Role-gated desired-state writes; NTS remains disabled; global `ntpd` apply only | Pass D/N/K/F/Z/V | E1–E8 |
