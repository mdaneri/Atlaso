...
rows (200). Collection metadata identifies which limits discarded evidence. Journal sources that reach the selected
event limit are conservatively marked truncated because older matching events may exist; their evidence retains the
...
On the appliance, publication and deletion flush the spool directory before committing lifecycle metadata.
A failed publication flush cannot advertise a ready bundle; a failed deletion flush remains retryable.
Corrupt archives or missing manifests produce an unavailable-evidence response when inspected.
