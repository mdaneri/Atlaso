...
opens a two-step Definition and Review wizard; double-clicking an existing row or choosing **Edit** from its context
menu opens the same wizard, while **Remove** deletes the definition after confirmation. Each row records the variable
...
override exists, Atlaso renders the configured default. Removing a referenced definition makes the Kickstart invalid
