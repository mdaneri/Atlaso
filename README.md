...
- [Appliance Update](docs/operate/appliance-update.md) — inspect configured Photon, PowerShell, and signed Atlaso
  ...
  publishes the immutable wheel handoff, then the protected software publisher signs the complete `vX.Y.Z` bundle and
  advances `development`. Promotion to `preview` or `stable` and all OVA/virtualization production remain explicit
  manual operations.
...
rolls back only invocation-owned artifacts. Before rollback removal begins, the wrapper durably records the exact
stopped VMX, quarantine, and host-boot ownership so a surviving removal tree blocks same-boot reruns. After encrypted
import is proven, it stops that exact VM, removes and
