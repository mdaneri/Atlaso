...
def test_helper_rejects_commands_paths_and_unbounded_windows(monkeypatch):
    ...
    for args in (["/etc/shadow"], ["journal-atlaso", "2026-01-01", "2026-01-02", "500"],
                 ["firewall-ipv4", "2026-01-01T00:00:00Z", "2026-03-01T00:00:00Z", "500"],
...
def test_helper_invokes_only_installed_isolated_collector(monkeypatch, capsys):
    ...
    def run(args, **kwargs):
        ...
        return SimpleNamespace(returncode=0, stdout=json.dumps({"source": "firewall-ipv4", "evidence": {"policies": {"INPUT": "DROP"}}}).encode())

    monkeypatch.setattr(helper.subprocess, "run", run)
    assert helper.main(["atlaso-helper", "diagnostics", "source", "firewall-ipv4", "2026-01-01T00:00:00Z", "2026-01-02T00:00:00Z", "500"]) == 0
