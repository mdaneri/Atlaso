...
if ($childArguments -contains 'caller-controlled-value-must-not-forward') {
    throw 'The bridge nonce must be generated by the bridge and must not be forwarded from caller parameters.'
}
...
if (-not $scriptText.Contains('[guid]::NewGuid().ToString(''N'')', [System.StringComparison]::Ordinal) -or
