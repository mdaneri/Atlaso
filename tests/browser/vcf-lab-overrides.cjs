...
(async () => {
  ...
  try {
    for (const viewport of [{width: 1600, height: 1000}, {width: 900, height: 1200}, {width: 430, height: 800}]) {
      ...
      tlsAvailable = true;
      await page.locator('[data-lab-action="probe"]').click();
      await page.waitForFunction(() => document.querySelector('[data-lab-tls]').textContent === 'tls-fixture');
      ...
      await page.locator('[data-atlaso-wizard-submit]').click();
      await page.locator('[data-lab-task]').waitFor({state:'visible'});
      assert.deepEqual(calls.filter((call) => call.operation === 'execute').at(-1).body.credentials, {api:'synthetic-api',ssh:'synthetic-ssh',root:'synthetic-root'});
      for (const kind of ['api', 'ssh', 'root']) assert.equal(await page.locator(`[name="${kind}_password"]`).inputValue(), '');
      assert.equal(await page.evaluate(() => localStorage.length + sessionStorage.length), 0);
