...
def test_vmware_release_snapshot_preserves_attached_or_detached_identity() -> None:
    ...
    assert (
        "[bool]$recheckedTaskSourceIdentity.Detached -ne $taskSourceDetached"
        in snapshot_recheck
    )
    assert "-not $taskSourceDetached" in snapshot_recheck
    assert "$recheckedTaskSourceIdentity.Branch -cne $taskSourceBranch" in snapshot_recheck
...
def test_lifecycle_vmware_script_supports_routing_wan_only_and_esxi_pxe_install():
    ...
    assert "$applianceWheelIdentity = Sync-ApplianceApplicationWheel -ApplianceVmx $applianceVmx" in runner
    assert "Save-ApplianceDeploymentIdentity -ApplianceVmx $applianceVmx -Wheel $applianceWheelIdentity" in runner
