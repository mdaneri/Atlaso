...
def test_pwa_manifest_service_worker_and_offline_shell(client):
    ...
    assert 'const ATLASO_CACHE = `${ATLASO_CACHE_PREFIX}316`;' in service_worker.text
    ...
    assert "/static/app.js?v=issue-779-4" in service_worker.text
...
def test_shared_ui_pattern_shell_and_wizard_contracts(client):
    ...
    for shell, app_asset in (
        (base, "/static/app.js?v=issue-779-4"),
...
def test_monitor_page_renders_template_and_browser_assets(client):
    ...
    assert "/static/app.js?v=issue-779-4" in page.text
    app_css = client.get("/static/app.css")
    assert app_css.status_code == 200
