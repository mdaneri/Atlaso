...
def test_pwa_manifest_service_worker_and_offline_shell(client):
    ...
    assert "atlaso-pwa-v173" in service_worker.text
    ...
    assert "/static/app.js?v=atlaso-oidc-admin-20260726-2" in service_worker.text
...
def test_shared_ui_pattern_shell_and_wizard_contracts(client):
    ...
    for shell in (base, public_base):
        ...
        assert shell.index(
            "/static/ui-patterns.js?v=atlaso-ui-foundation-20260726-4"
        ) < shell.index("/static/app.js?v=atlaso-oidc-admin-20260726-2")
...
def test_monitor_page_renders_and_data_endpoint(client):
    ...
    assert "/static/app.js?v=atlaso-oidc-admin-20260726-2" in page.text
    app_css = client.get("/static/app.css")
    assert app_css.status_code == 200
