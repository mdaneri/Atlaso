    ...
    def fake_guestfish(_disk: Path, commands: list[str]) -> list[str]:
        ...
        for command in commands:
            if not command.startswith("download "):
                continue
            guest, destination = command.removeprefix("download ").split(" ", 1)
            relative = guest.split("/site-packages/", 1)[1]
            Path(destination).write_bytes(members[relative])
        return []

    monkeypatch.setattr(verifier, "_guestfish", fake_guestfish)
    digest = hashlib.sha256(wheel.read_bytes()).hexdigest()
    result = verifier.verify_installed_wheel(assets, wheel, digest)
    assert result["wheel_sha256"] == digest
    assert result["files_verified"] == len(members)
...
def test_rejects_altered_installed_wheel_member(
    ...
    def fake_guestfish(_disk: Path, commands: list[str]) -> list[str]:
        ...
        for command in commands:
            ...
            content = (
                b"substituted"
                if relative == "atlaso/__init__.py"
    ...
    digest = hashlib.sha256(wheel.read_bytes()).hexdigest()
    with pytest.raises(
        SystemExit, match="installed Atlaso wheel member does not match"
    ):
        verifier.verify_installed_wheel(assets, wheel, digest)
