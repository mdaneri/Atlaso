...
def set_applied_pxe_runtime(
    ...
    *,
    ...
    Args:
        ...
        kickstarts: Applied Kickstart revisions supplied to the test scenario.
        hosts: Applied ESXi host definitions supplied to the test scenario.
        custom_variables: Applied custom-variable definitions supplied to the test scenario.
    """
...
def test_esxi_boot_capability_authorization_binding_and_single_use(
    ...
    from atlaso.app.database import SessionLocal
    from atlaso.app.models import DhcpScope, EsxiKickstart
    from atlaso.app.services.esxi_pxe import (
    ...
    http_base = tmp_path / "http" / "esxi"
    mac_key = "01-00-50-56-aa-bb-cc"
    image_key = "esxi-test-image"
    http_host_root = http_base / mac_key
    tftp_host_root = tftp_root / mac_key
    http_host_root.mkdir(parents=True)
    tftp_host_root.mkdir(parents=True)
    (http_host_root / "mboot.efi").write_bytes(b"test mboot")
    boot_cfg = "prefix=http://192.168.50.1:8080/pxe/esxi/images/esxi-test-image\nkernel=b.b00\nkernelopt=runweasel\nmodules=jumpstrt.gz\n"
    (http_host_root / "boot.cfg").write_text(boot_cfg, encoding="utf-8")
    (tftp_host_root / "boot.cfg").write_text(boot_cfg, encoding="utf-8")

    monkeypatch.setattr(network_boot, "ESXI_TFTP_ROOT", tftp_root)
    monkeypatch.setattr(network_boot, "ESXI_PXE_HTTP_BASE", http_base)
    monkeypatch.setattr(
        network_boot,
        "kickstart_vault_values_for_markers",
        lambda _db, _names: {"lab.root.password": "resolved-value-for-test"},
    )
    ...
    with SessionLocal() as db:
        ...
        assign_kickstart_content(
            kickstart,
            "install --firstdisk={{custom.disk}}\nnetwork --bootproto=static --ip={{host.ip_address}}\nrootpw {{vault.lab.root.password}}\nreboot\n",
            max_bytes=262_144,
        )
        host = EsxiPxeHost(
            hostname="esx-capability",
            mac_address="00:50:56:aa:bb:cc",
            ip_address="192.168.50.150",
            kickstart_id=kickstart.id,
            variables_json=host_variables_json({"custom.disk": "firstdisk"}),
        ...
        applied_artifact = {
            ...
            "kickstart_id": kickstart.id,
            "http_boot_cfg_path": str(http_host_root / "boot.cfg"),
            "uefi_tftp_boot_cfg_path": str(tftp_host_root / "boot.cfg"),
            "pxelinux_config_path": str(tftp_root / "pxelinux.cfg" / mac_key),
            "is_default": False,
        }
    ...
    old_hash = client.get(
        f"/pxe/esxi/ks/{revision[:12]}.cfg?mac={mac_key}",
        headers={"host": "192.168.50.1:8080"},
    )
...
def test_wake_on_lan_packet_and_distinct_broadcast_delivery():
    """Verify that wake on lan packet and distinct broadcast delivery."""
...
def test_wake_on_lan_preserves_targets_sent_before_udp_failure():
    """Verify that wake on lan preserves targets sent before udp failure."""
    calls = []
...
def test_wake_on_lan_preserves_retryable_error_before_any_delivery():
    """Verify that wake on lan preserves retryable error before any delivery."""
    class FakeSocket:
        """Represent fake socket."""
        def __enter__(self):
            """Enter the managed context.
...
def test_inventory_report_is_bounded_and_uses_mac_for_placeholder_uuid():
    """Verify that inventory report is bounded and uses mac for placeholder uuid."""
...
def test_inventory_report_v1_normalizes_to_v2_compatibility_shape():
    """Verify that inventory report v1 normalizes to v2 compatibility shape."""
...
def test_complete_inventory_report_v2_normalizes_all_structured_hardware():
    """Verify that complete inventory report v2 normalizes all structured hardware."""
...
def test_inventory_report_rejects_dimm_string_and_report_size_limits():
    """Verify that inventory report rejects dimm string and report size limits."""
    payload = inventory_report_v2()
...
def test_inventory_report_rejects_normalized_report_size_limit():
    """Verify that inventory report rejects normalized report size limit."""
    payload = inventory_report_v2()
...
def test_inventory_report_ignores_optional_placeholder_interface_macs():
    """Verify that inventory report ignores optional placeholder interface macs."""
    payload = inventory_report()
...
def test_prune_superseded_shredos_media_waits_for_applied_manifest(
    ...
    def write_snapshot(path: Path, digest: str) -> dict:
        ...
        Args:
            ...
            digest: Digest supplied by the caller.

        Returns:
            The write snapshot result.
        """
...
def test_public_rate_limit_prunes_expired_client_keys():
    """Verify that public rate limit prunes expired client keys."""
...
def test_esxi_menu_requires_authorization_before_firmware_loader(db_session):
    """Verify that ESXi firmware loaders stay unavailable without authorization.
...
def test_boot_media_downloader_rejects_https_downgrade_redirect():
    """Verify that boot media downloader rejects https downgrade redirect."""
...
def test_checksum_filename_accepts_publisher_relative_path():
    """Verify that checksum filename accepts publisher relative path."""
    digest = "a" * 64
...
def test_inventory_linux_build_enables_reproducible_mode():
    """Verify that inventory linux build enables reproducible mode."""
...
def _write_test_media_cache(
    ...
    *,
    ...
    """Persist test media cache.

    Args:
        directory: Directory supplied by the caller.
...
def test_media_staging_lease_serializes_marker_publication_with_recovery(
    ...
    def recording_open(path, flags, mode=0o777):
        """Return recording open.

        Args:
            path: Filesystem or URL path to read, validate, or update.
            flags: Flags supplied by the caller.
