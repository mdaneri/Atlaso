...
- after an authorized merge and all remaining activity, send the primary-checkout controller a `cleanup-ready` handoff
  and require the ordered terminal states `validation_resources_released`, `remote_branch_absent`, `worktree_removed`,
  and `task_title_done`; delete only
  ...
  coordinated release, disclosure, and authorized advisory-state activity. Fulfill the remote-branch state through
