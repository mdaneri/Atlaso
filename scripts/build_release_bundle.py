...
def project_version(root: Path = ROOT) -> str:
    ...
    Args:
        root: Repository source root whose version should be read.
...
def copy_source(target: Path, *, source_root: Path = ROOT) -> None:
    """Handle copy source.

    Args:
        target: Target resource or location affected by the operation.
        source_root: Repository source root to copy.
    """
    shutil.copytree(
        source_root,
        target,
...
def build_application_wheel(
    target: Path, *, commit: str, built_at: str, source_root: Path = ROOT
) -> Path:
    ...
    Args:
        ...
        source_root: Repository source root to package.
    ...
    wheels = list(wheel_dir.glob("atlaso-*.whl"))
    if len(wheels) != 1:
        raise SystemExit("release build must produce exactly one Atlaso wheel")
    return wheels[0]
