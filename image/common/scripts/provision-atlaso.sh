...
# Preserve a stable controller-path identity under /dev/disk/by-id for every
# virtual disk so first boot can bind destructive initialization to an exact
# hypervisor slot instead of a transient /dev/sdX name.
SUBSYSTEM=="block", ENV{DEVTYPE}=="disk", IMPORT{builtin}="path_id", ENV{ID_PATH_TAG}=="?*", SYMLINK+="disk/by-id/atlaso-path-$env{ID_PATH_TAG}"
EOF
...
case "$ATLASO_GUEST_PLATFORM" in
  hyperv)
    atlaso_depot_scsi_tuple='0:0:1'
    atlaso_backup_scsi_tuple='0:0:2'
    atlaso_system_scsi_tuple=''
    ;;
  vmware)
    atlaso_depot_scsi_tuple='0:2:0'
    atlaso_backup_scsi_tuple='0:3:0'
    atlaso_system_scsi_tuple='0:1:0'
    ;;
  *)
    printf 'Unsupported Atlaso data-disk platform: %s\n' "$ATLASO_GUEST_PLATFORM" >&2
    exit 1
    ;;
esac
install -d -o root -g root -m 0755 /etc/atlaso
cat >/etc/atlaso/data-disks.conf <<EOF
# Generated by the Atlaso image build. First boot fails closed unless both
# fixed-capacity data disks match these hypervisor SCSI identities.
ATLASO_DATA_DISK_SIZE_BYTES=536870912000
ATLASO_DEPOT_SCSI_TUPLE=$atlaso_depot_scsi_tuple
ATLASO_BACKUP_SCSI_TUPLE=$atlaso_backup_scsi_tuple
ATLASO_SYSTEM_SCSI_TUPLE=$atlaso_system_scsi_tuple
EOF
chown root:root /etc/atlaso/data-disks.conf
chmod 0644 /etc/atlaso/data-disks.conf
