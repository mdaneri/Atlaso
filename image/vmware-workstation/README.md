...
directory and returns an actionable failure. Cleanup checks the running and registered Workstation inventories, stops
and unregisters only when needed, verifies both transitions, and preserves all artifacts if `vmrun` fails or returns
unverifiable state. Pass `-IncludeLabNetworkAdapters` only after `VMnet2`, `VMnet3`, and `VMnet4` exist for the
