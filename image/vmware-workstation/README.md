...
auto-removal disabled. It then reinstalls the `ntpsec` and `python3-ntp` runtime closure after build-only cleanup and
again after the final Photon update. The final runtime verifier requires both packages, preventing a reusable image
without `ntpd.service` from completing successfully. It also requires valid `/etc/os-release` and
`/etc/photon-release` files, verifies TDNF's
...
removes empty ordinary `*.lck` directories left when Packer deletes their lock files. A no-follow Windows deletion
